...
public class ReceitasSemantico extends ReceitasBaseVisitor<Void> {
    ...
    public enum DimensaoUnidade {
        MASSA,
        VOLUME,
        UNIDADE
    }

    private DimensaoUnidade dimensaoDe(String unidadeTexto) {
        return switch (unidadeTexto) {
            case "g" -> DimensaoUnidade.MASSA;
            case "ml" -> DimensaoUnidade.VOLUME;
            case "un" -> DimensaoUnidade.UNIDADE;
            case "kg" -> DimensaoUnidade.MASSA;
            case "l" -> DimensaoUnidade.VOLUME;
            default -> null; // ou lance IllegalArgumentException
        };
    }

    private static final Set<String> LIQUIDOS = Set.of(
            "leite", "oleo", "agua", "vinagre", "vinho", "caldo_de_frango", "caldo_de_carne", "caldo_de_legume",
            "azeite", "shouyo", "tare");
    ...
    public Void visitDeclaracao_ingredientes(ReceitasParser.Declaracao_ingredientesContext ctx) {
        for (var ingCtx : ctx.lista_ingredientes().ingrediente()) {
            ...
            String unidadeTexto = ingCtx.unidade().getText(); // "g", "ml" ou "un"
            DimensaoUnidade dimReal = dimensaoDe(unidadeTexto);
            ...
            // 2) verifica se faz sentido a dimensão
            if (dimReal == DimensaoUnidade.VOLUME && !LIQUIDOS.contains(nome)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(tk,
                        "ML/L  não esperado em '" + nome + "'.\n");
            }
            if (dimReal == DimensaoUnidade.MASSA && LIQUIDOS.contains(nome)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(tk,
                        "ML/L era esperado em '" + nome + "'.\n");
            }
    ...
    public Void visitDeclaracao_receita(ReceitasParser.Declaracao_receitaContext ctx) {
        ...
        // 2.4) Ingredientes da receita (incluindo checagem dimensional)
        Set<String> usados = new HashSet<>();
        for (var ingReq : ctx.lista_ingredientes().ingrediente()) {
            Token tkIng = ingReq.NOME().getSymbol();
            String nomeIng = tkIng.getText();
            String uniTexto = ingReq.unidade().getText(); 
            DimensaoUnidade dimReal = dimensaoDe(uniTexto);

            // a) deve existir no inventário
            if (!tabela.existeIngrediente(nomeIng)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(
                        tkIng,
                        "Ingrediente '" + nomeIng + "' não declarado.\n");
            }
            // b) sem duplicatas na mesma receita
            ...
            // c) checagem de sentido físico da unidade
            if (dimReal == DimensaoUnidade.VOLUME && !LIQUIDOS.contains(nomeIng)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(
                        tkIng,
                        "ML/L  não esperado em '" + nomeIng + "'.\n");
            }
            if (dimReal == DimensaoUnidade.MASSA && LIQUIDOS.contains(nomeIng)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(
                        tkIng,
                        "ML/L era esperado em '" + nomeIng + "'.\n");
            }
            if (dimReal == DimensaoUnidade.UNIDADE && LIQUIDOS.contains(nomeIng)) {
                ReceitasSemanticoUtils.adicionarErroSemantico(
                        tkIng,
                        "ML/L era esperado em '" + nomeIng + "'.\n");
            }
